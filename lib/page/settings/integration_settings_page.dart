...
class IntegrationSettingsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SettingsScreen(title: "Integration", children: [
  ...
  Future<Widget> _addIntegration(String s, BuildContext context) async {
    ...
    if (fieldData.hasError()) {
      return SimpleTextSettingsTile(
          title: "Could not create $s", subtitle: fieldData.error);
    }
    var fields =
        String.fromCharCodes(fieldData.data ?? List.empty()).split(";");
    return SettingsGroup(title: s, children: [
      SwitchSettingsTile(
        topPadding: 0.0,
        title: 'Activate $s',
                ...
                showDialog(
                    ...
                    builder: (context) =>
                        LoginWidget(fields, s, onLogin: (p0) async {
                          var result =
                              await MethodChannelService.loginFor(s, p0);
                          if (result) {
